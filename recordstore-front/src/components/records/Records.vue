      ...
      <li class="py-4" v-for="record in records" :key="record.id" :record="record">
        <div class="flex items-center justify-between flex-wrap">
          <div class="flex-1 flex justify-between flex-wrap pr-4">
            ...
            <p class="block font-mono font-semibold">{{ getArtistRecord(record) }}</p>
          </div>
          ...
          <button class="bg-transparent text-sm hover:bg-red hover:text-white text-red border border-red no-underline font-bold py-2 px-4 mr-2 rounded" @click.prevent="removeRecord(record)">Delete</button>
        </div>
          ...
          <form @submit.prevent="updateRecord(record)">
            <div class="mb-6 p-4 bg-white rounded border border-grey-light mt-4">
              ...
              <div class="mb-6">
                ...
                <input type="text" class="input" v-model="record.title" id="title">
              </div>
              <div class="mb-6">
                <label for="year" class="label">Year</label>
                <input type="text" class="input" v-model="record.year" id="year">
              </div>

              <div class="mb-6">
                <label for="artist_update" class="label">Artist</label>
  ...
  created () {
    if (!localStorage.signedIn) {
      ...
      this.$http.secured.get('/api/v1/records')
        ...
        .catch(error => this.setError(error, 'Can not load records'))
      this.$http.secured.get('/api/v1/artists')
        .then(response => { this.artists = response.data })
        .catch(error => this.setError(error, 'Can not load artists'))
    }
  },
  methods: {
    setError (error, text) {
    ...
    getArtist (record) {
      ...
      let artist
      recordArtistValues.forEach((artist) => {
        artist = artist.name
      })
      return artist
    },
    addRecord () {
      ...
      if (value) {
        this.$http.secured.post('/api/v1/records', { records: { title: this.newRecord.title, year: this.newRecord.year, artist_id: this.newRecord.artist } })
          .then(response => {
            this.records.push(response.data)
            this.newRecord = ''
          })
          .catch(error => this.setError(error, 'Can not create record'))
      }
    },
    removeRecord (record) {
      this.$http.secured.delete(`/api/v1/records/${record.id}`)
        ...
        .catch(error => this.setError(error, 'Can not delete record'))
    },
    editRecord (record) {
    ...
    updateRecord (record) {
      ...
      this.$http.secured.patch(`/api/v1/records/${record.id}`, { record: { title: record.title, year: record.year, artist_id: record.artist } })
        .catch(error => this.setError(error, 'Can not update record'))
    }
  }
}
</script>

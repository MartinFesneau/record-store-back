  ...
  <div class="max-w-md m-auto py-10">
    ...
    <form @submit.prevent="addArtist">
      <div class="mb-6">
        ...
        >
        <input type="submit" value="add Artist" class="font-sans font-bold px-4 rounder cursor-pointer no-underline bg-green hover:bg-green-dark block w-full py-4 text-white item-center justify-center">
      </div>
    </form>
      ...
      <li class="py-4" v-for="artist in artists" :key="artist.id" :artist="artist">
        <div class="flex items-center justify-between flex-wrap">
          <p class="block flex-1 font-mono font-semibold flex items-center"><svg class="fill-current text-indigo w-6 h-6 mr-2" viewBox="0 0 20 20" width="20" height="20"><title>music artist</title><path d="M15.75 8l-3.74-3.75a3.99 3.99 0 0 1 6.82-3.08A4 4 0 0 1 15.75 8zm-13.9 7.3l9.2-9.19 2.83 2.83-9.2 9.2-2.82-2.84zm-1.4 2.83l2.11-2.12 1.42 1.42-2.12 2.12-1.42-1.42zM10 15l2-2v7h-2v-5z"></path></svg>
            ...
            <button class="bg-transparent text-sm hover:bg-red hover:text-white text-red border border-red no-underline font-bold py-2 px-4 mr-2 rounded" @click.prevent="removeArtist(artist)">Delete</button>
        ...
        <div v-if="artist === editedArtist">
          <form @submit.prevent="updateArtist(artist)">
            <div class="mb-6 p-4 bg-white rounded border border-grey-light mt-4">
              ...
              <input type="submit" value="udpate" class="my-2 bg-transparent text-sm hover:bg-blue hover:text-white text-blue border border-blue no-underline font-bold py-2 px-4 rounded cursor-pointer">
            </div>
          </form>
        </div>
    ...
    addArtist () {
      ...
      if (value) {
        this.$http.secured.post('/api/v1/artists', { artist: { name: this.newArtist.name } })
          ...
          .catch(error => this.setError(error, 'Can not create artist'))
      }
    },
    removeArtist (artist) {
      this.$http.secured.delete(`/api/v1/artists/${artist.id}`)
        ...
        .catch(error => this.setError(error, 'Can not delete artist'))
    },
    editArtist (artist) {
